...
"""
银行余额调节表自动化生成工具 v2.0
用途：将王宇维护的银行到款明细表转换为用友系统导入格式

改进：
- 完整的月份识别（自动处理28/30/31天）
- 数据验证机制
- 错误处理和日志记录
- 数据完整性检查和对账功能
- 大数据量优化
...
class BankStatementGenerator:
    ...
    def get_month_info(self, target_month):
        ...
        try:
            ...
            last_day = calendar.monthrange(year, month)[1]  # 自动获取该月天数
    ...
    def read_sheet_data(self, sheet_name):
        ...
        for row_idx in range(3, sheet.nrows):
            try:
                ...
                remark = sheet.cell_value(row_idx, 2)
                income = sheet.cell_value(row_idx, 4)
                expense = sheet.cell_value(row_idx, 5)
...
def main():
    """主程序"""
    print("=" * 60)
    print("银行余额调节表自动化生成工具 v2.0")
    print("=" * 60)

    # 默认参数
    default_input = "/Users/mi/Downloads/本地2025年天地银行到款明细表188版.xls"
    default_month = "2025-11"
    default_output = "/Users/mi/Desktop/财务/所有银行_2025年11月_对账单.xls"
    ...
    # 创建生成器
    generator = BankStatementGenerator(input_file)
