...
# 默认文件路径
DEFAULT_INPUT_FILE = '/Users/mi/Downloads/2025年镭海易墅建行到款明细表 .xls'


class LeihaiStatementGenerator:
    """镭海银行对账单生成器"""
    ...
    def read_sheet_data(self, sheet_name):
        ...
        # 镭海数据结构：从第4行开始（第0行标题，第1行表头，第2行期初余额，第3行开始数据）
        # 列顺序：[日期(0), 项目明细(1), 结算方式(2), 收入(3), 支出(4), 余额(5), ...]
        for row_idx in range(3, sheet.nrows):
            try:
                date_cell = sheet.cell_value(row_idx, 0)  # 日期
                account_name = sheet.cell_value(row_idx, 1)  # 项目明细
                income = sheet.cell_value(row_idx, 3)  # 收入
                expense = sheet.cell_value(row_idx, 4)  # 支出
...
def main():
    """主程序"""
    print("=" * 60)
    print("镭海 - 银行对账单生成工具 v2.0")
    print("=" * 60)

    # 默认参数
    default_input = DEFAULT_INPUT_FILE
    default_month = "2025-11"
    default_output = "/Users/mi/Desktop/财务/镭海_2025年11月_对账单.xls"
    ...
    if not output_file:
        ...
        output_file = f"/Users/mi/Desktop/财务/镭海_{month_str}_对账单.xls"
    ...
    # 创建生成器
    generator = LeihaiStatementGenerator(input_file)
